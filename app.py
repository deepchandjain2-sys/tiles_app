...
def generate_pdf_quotation(customer_info, items_list):
    ...
    pdf.cell(28, 7, "Area", 1, 0, "C", fill=True)
    pdf.cell(50, 7, "Tile Item", 1, 0, "L", fill=True)
    pdf.cell(16, 7, "Con Fac (H)", 1, 0, "C", fill=True)
    pdf.cell(14, 7, "Pack (I)", 1, 0, "C", fill=True)
    pdf.cell(20, 7, "Total Sq.Ft", 1, 0, "R", fill=True)
    pdf.cell(18, 7, "Req. Boxes", 1, 0, "R", fill=True)
...
# --- PAGE 3: SQFT ENTRY & FINAL ESTIMATE ---
    ...
    wa_msg = f"🏢 *JAY GRANITE & TILES - {curr_c.get('branch', 'HIRIYUR').upper()} SHOWROOM*\n"
    ...
    wa_msg += f"please update physical stock as possible!"
