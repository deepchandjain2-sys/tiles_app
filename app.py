...
# -------------------------------------------------------------
# 2. PROPER AUTHENTICATION SYSTEM (WORKING LOGOUT)
# -------------------------------------------------------------
...
if not st.session_state.authenticated:
    ...
    col_log, _ = st.columns([1.2, 2])
    with col_log:
        st.subheader("🔐 Staff Login")
        entered_pin = st.text_input("Enter Passcode / PIN", type="password", key="login_pin_input")
...
# -------------------------------------------------------------
# 3. DIRECT GOOGLE SHEET CSV FETCH (FIXES 404 ERROR)
# -------------------------------------------------------------
...
# -------------------------------------------------------------
# 5. TILE MULTI-SELECTION HUB (DIRECT SQFT + LXW)
# -------------------------------------------------------------
if "Tile Multi-Selection Hub" in nav_option:
    ...
    <div class='metric-banner'>
        💡 Area: <b>{net_sqft:.2f} Sq.Ft</b> | Box Estimate: <b>{exact_boxes:.2f} Boxes</b> (Order: <b>{rounded_boxes} Boxes</b> = {total_coverage_delivered:.2f} Sq.Ft)
    </div>
    """, unsafe_allow_html=True)
