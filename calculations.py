...
if st.session_state.role == "admin":
    st.session_state.branch = st.sidebar.selectbox(
        ...
        ["All Showrooms", "Hiriyur", "Davangere", "New Show Room"],
        index=["All Showrooms", "Hiriyur", "Davangere", "New Show Room"].index(st.session_state.branch)
    )
else:
...
if nav == "1️⃣ Customer Registration & History":
    ...
    with tab_new:
        with st.form("new_cust_form"):
            ...
            c_eng = st.text_input("Engineer Name")
            c_eng_mob = st.text_input("Engineer Mobile Number")
    ...
    with tab_existing:
        if filtered_clients:
            ...
            with c_info1:
                ...
                st.write(f"**Engineer:** {chosen_cust.get('engineer', 'N/A')} ({chosen_cust.get('engineer_mobile', 'N/A')})")
...
# --- PAGE 2: TILE SELECTION ---
    ...
    st.info(f"👤 Active Client: **{curr_c['name']}** (#{curr_c['id']} - {curr_c['mobile']}) | 🏢 Branch: **{curr_c.get('branch', 'Hiriyur')}** | 👔 Staff: **{curr_c.get('salesman', st.session_state.username)}**")
    
    col1, col2, col3 = st.columns(3)
    with col1:
        floor_options = ["Ground Floor", "1st Floor", "2nd Floor", "3rd Floor", "✏️ Custom Floor"]
        sel_floor = st.selectbox("Floor Level", floor_options)
        final_floor = st.text_input("Custom Floor Name", "Terrace / Basement") if sel_floor == "✏️ Custom Floor" else sel_floor
    with col2:
        surface = st.radio("Category", ["Floor", "Wall"], horizontal=True)
    with col3:
        if surface == "Floor":
            area_options = [
                "Hall", "Kitchen", "Bedroom 1", "Bedroom 2", "Bedroom 3",
                "Bathroom 1", "Bathroom 2", "Bathroom 3", "Pooja Room", "Parking", "Gallery", "✏️ Custom Area"
            ]
        else:
            area_options = [
                "Bathroom 1", "Bathroom 2", "Bathroom 3", "Pooja Wall", "Front Wall", "Kitchen Dado", "✏️ Custom Area"
            ]
        sel_area = st.selectbox("Select Area", area_options)
        final_area = st.text_input("Custom Area Name", "Staircase") if sel_area == "✏️ Custom Area" else sel_area
        
    search = st.text_input("🔍 Search Design from Google Sheet Catalog (Code / Name)", "")
    ...
    st.success(f"📐 **Catalog Specs:** Con Factor (H): {cf} | Packing Unit (I): {pu} | **Coverage: {box_cov:.2f} Sq.Ft / Box**")
    
    if st.button("➕ Add to Selection Queue", type="primary", use_container_width=True):
        new_item = {
        ...
        st.success(f"✅ **{chosen_tile}** ({final_area}) queue mein add ho gayi!")
        st.rerun()

    st.markdown("---")
    saved_items = curr_c.get("selections", [])
    st.subheader(f"🛒 Queued Items ({len(saved_items)})")
    
    if saved_items:
        disp_df = pd.DataFrame(saved_items)[["floor", "surface", "area", "tile", "con_factor", "packing_unit"]]
        st.dataframe(disp_df.rename(columns={"floor": "Floor Level", "surface": "Category", "area": "Area", "tile": "Tile Item", "con_factor": "Con Factor", "packing_unit": "Packing"}), use_container_width=True)
        st.info("👉 Next step ke liye sidebar se **'3️⃣ Sq.Ft Entry & Final Estimate'** par jayein.")
    else:
        st.caption("Queue khaali hai.")
...
# --- PAGE 3: SQFT ENTRY & FINAL ESTIMATE ---
    ...
    curr_c = st.session_state.current_customer
    st.title("📐 Measurement Calculation & Final BOQ")
    st.info(f"👤 Active Client: **{curr_c['name']}** (#{curr_c['id']} - {curr_c['mobile']}) | 🏢 Branch: `{curr_c.get('branch', 'Hiriyur')}`")
    
    saved_items = curr_c.get("selections", [])
    if not saved_items:
        st.warning("Is customer ke paas koi selection queue mein nahi hai.")
        st.stop()
        
    st.markdown("### ✏️ Enter Square Feet for Each Selected Item:")
    
    h_col1, h_col2, h_col3, h_col4 = st.columns([5, 2.5, 2.5, 1.5])
    h_col1.markdown("**Location & Tile**")
    h_col2.markdown("**Manual Sq.Ft**")
    ...
    for it in saved_items:
        ...
        with c1:
            ...
            st.caption(f"🧱 *{tile_name}* | (1 Box = {cov_per_box:.2f} Sq.Ft | Con: {cf}, Pack: {pu})")
    ...
    k1.metric("Total Items", len(curr_c["selections"]))
    ...
    with b3:
        if st.button("✅ Finalize Deal & Clear Queue", type="primary", use_container_width=True):
            curr_c["status"] = "FINALIZED"
            curr_c["total_sqft"] = tot_sq
            curr_c["total_boxes"] = tot_bx
            update_customer_db(curr_c)
            st.session_state.current_customer = None
            st.success(f"🎉 **{curr_c['name']}** ka order finalize ho gaya aur queue clear kar di gayi hai!")
            st.rerun()
